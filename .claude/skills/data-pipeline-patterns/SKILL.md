...
## dbt SQL Style & sqlfluff Compliance

CI runs `sqlfluff lint 02_dbt/models/ --dialect bigquery` on every PR.
Apply these rules when writing every dbt model — violations block merges.

### LT01 — Single space before `as`

Never use extra spaces to align column aliases into visual columns.
...
SQL reserved words used as column aliases trigger RF04. Common offenders
in this codebase: `name`, `date`, `description`, `category`, `value`,
`type`, `schema`, `key`.

**Option 1 — rename** (preferred when it adds clarity):
```sql
cast(name as string) as coin_name,
```

**Option 2 — suppress inline** (use only in staging views where the
alias must match the raw source column exactly):
```sql
...
Window function `over (...)` clauses are the most common offender.
Always wrap them:
...
Keep inline comments under 80 chars too — split long ones across lines.

### Quick checklist before committing any dbt model

- [ ] No multi-space gaps before `as` in SELECT lists
- [ ] Column aliases that are SQL keywords use `-- noqa: RF04` or are renamed
- [ ] `over (...)` clauses are wrapped when the line would exceed 80 chars
