...
with base as (
    ...
    from {{ ref('wh_indices_daily') }} as d
    inner join {{ ref('wh_indices_returns') }} as r
        on
...
with_windows as (
    ...
    select
        ...
        -- 20-day simple moving average (null for first 19 trading days)
        avg(close) over (
        ...
        -- 20-day annualized realized vol: stddev(daily_return) * sqrt(252)
        -- null for first 19 trading days or when daily_return is null
